...
import { Form, InputNumber, DatePicker, Radio, Switch, Card, Tooltip, TimePicker, Row, Col, Button, Space, List, message } from 'antd';
import { 
  DollarOutlined, 
  CalendarOutlined,
...
import { calculateWorkingDaysPerMonth, calculateTotalBreakMinutes, hasOverlappingBreaks, isTimeInWorkRange, convertTimeToMinutes } from '@/utils/salaryCalculator';
...
const SalaryForm = ({ onValuesChange }: SalaryFormProps) => {
  ...
  const getDisabledTime = (breakId: string, type: 'start' | 'end') => {
    return (_: Dayjs, panel: string) => {
      // 如果是在选择面板，传入当前所选时间
      const currentValue = _ ? _.format('HH:mm') : undefined;
  ...
  const handleCustomWorkingDaysChange = (checked: boolean) => {
    ...
    if (!checked) {
      ...
      // 通知父组件值变化
      const allValues = form.getFieldsValues();
